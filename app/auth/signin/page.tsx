"use client";

import { signIn } from "next-auth/react";

export default function SignInPage() {
  return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-blue-50 to-blue-100 p-4">
      <div className="bg-white/80 backdrop-blur shadow-xl rounded-2xl px-8 py-10 max-w-md w-full flex flex-col items-center gap-6">
        <h2 className="text-2xl font-bold text-blue-700 mb-1 text-center">
          Welcome to the Job Board
        </h2>
        <p className="text-gray-700 text-center mb-4">
          Sign in to post jobs or apply for opportunities
        </p>
        <button
          className="flex items-center gap-3 px-6 py-2 rounded-full bg-blue-600 text-white font-semibold shadow hover:bg-blue-700 transition w-full justify-center text-base cursor-pointer"
          onClick={() => signIn("github", { callbackUrl: "/" })}
        >
          <svg
            className="w-6 h-6"
            fill="currentColor"
            viewBox="0 0 24 24"
            aria-hidden="true"
          >
            <path d="M12 2C6.477 2 2 6.484 2 12.021c0 4.428 2.867 8.184 6.839 9.504.5.092.682-.217.682-.483 0-.237-.009-.868-.014-1.703-2.782.605-3.369-1.342-3.369-1.342-.454-1.154-1.11-1.462-1.11-1.462-.908-.62.069-.608.069-.608 1.004.07 1.532 1.032 1.532 1.032.892 1.53 2.341 1.088 2.91.832.092-.647.35-1.088.636-1.339-2.221-.253-4.555-1.113-4.555-4.951 0-1.093.39-1.988 1.029-2.688-.103-.253-.446-1.272.098-2.65 0 0 .84-.27 2.75 1.025A9.564 9.564 0 0 1 12 6.844c.85.004 1.705.115 2.504.337 1.909-1.295 2.748-1.025 2.748-1.025.546 1.378.203 2.397.1 2.65.64.7 1.028 1.595 1.028 2.688 0 3.847-2.337 4.695-4.566 4.944.359.309.678.919.678 1.852 0 1.336-.012 2.415-.012 2.744 0 .268.18.579.688.481C19.135 20.2 22 16.447 22 12.021 22 6.484 17.523 2 12 2z" />
          </svg>
          <span>Continue With Github</span>
        </button>
        <div className="text-xs text-gray-500 text-center mt-2">
          By signing in, you agree to our{" "}
          <a href="#" className="underline hover:text-blue-600 transition">
            Terms of Service
          </a>{" "}
          and{" "}
          <a href="#" className="underline hover:text-blue-600 transition">
            Privacy Policy
          </a>
          .
        </div>
      </div>
    </div>
  );
}
